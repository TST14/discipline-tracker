...
function RulesEditor({ habit, onClose }) {
  ...
  const addRule = () => {
    setRules(prev => [
      ...prev,
      {
        ...
        habit_id: habit.id,
        condition: 'lte',
        value: habit.scoring_type === 'time_of_day' ? '04:00' : '30',
        percentage: 100,
        rule_order: prev.length,
      },
    ])
  }
  ...
  const save = async () => {
    ...
    setError(null)
    try {
      const payload = rules.map((r, i) => ({ ...r, rule_order: i }))
      await setScoringRules(habit.id, payload)
      onClose()
    } catch {
        ...
        <>
          ...
          {rules.map((rule, idx) => (
            <div key={idx} className="grid grid-cols-[120px_100px_32px] gap-2 items-center">
              ...
              <button onClick={() => removeRule(idx)} className="text-gray-600 hover:text-red-400 transition-colors text-sm font-bold">✕</button>
            </div>
          ))}
          <button
          ...
          {rules.map((rule, idx) => (
            <div key={idx} className="grid grid-cols-[1fr_1fr_80px_32px] gap-2 items-center">
              ...
              <button
                ...
                className="text-gray-600 hover:text-red-400 transition-colors text-sm font-bold"
              >
                ✕
              </button>
            </div>
  ...
  return (
    <div className="bg-gray-800 rounded-xl p-4 space-y-3">
      ...
      <div className="grid grid-cols-2 gap-3">
        ...
        <div>
          <label className="block text-xs text-gray-500 mb-1">Max Points (Weightage)</label>
          <input
            type="number"
            min="0"
        ...
        <div>
          <label className="block text-xs text-gray-500 mb-1">Scoring Type</label>
          <select
            value={form.scoring_type}
            onChange={e => setForm(p => ({ ...p, scoring_type: e.target.value }))}
            className="w-full bg-gray-700 border border-gray-600 rounded-lg px-3 py-2 text-sm text-white focus:outline-none focus:ring-1 focus:ring-gray-500"
          >
            {SCORING_TYPES.map(t => (
...
export default function HabitConfig() {
  const [habits, setHabits] = useState([])
  const [editingHabit, setEditingHabit] = useState(null)   // habit obj | 'new' | null
  ...
  const load = async () => {
    try {
      setError(null)
      const h = await getHabits(false)   // all habits, including inactive
      setHabits(h)
    } catch {
              ...
              <div key={habit.id} className={`border-b border-gray-800/50 last:border-0 ${!habit.is_active ? 'opacity-50' : ''}`}>
                <div className="flex items-center gap-3 px-4 py-3">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="text-xs text-gray-500 mt-0.5">Max {habit.max_points} pts</div>
                  </div>
